...
def main(i):
    ...
    lowcut              = 0.75
    highcut             = 4
    video_path          = r'D:\ITB\Semester 7\Dataset VIPL-HR\p{}\v4\source3\video.avi'.format(i)
    video_cap           = cv2.VideoCapture(video_path)
    fs                  = video_cap.get(cv2.CAP_PROP_FPS) 
    window              = 400
    data_amount         = 600 #in frames
    ref_path            = r'D:\ITB\Semester 7\Dataset VIPL-HR\p{}\v4\source3\gt_HR.csv'.format(i)
    ...
    #Chrominance Method
    ppg_final = method.CHROM_method(smo_r, smo_g, smo_b, lowcut, highcut, fs)
    
    list_bpm_final = bpm.calculate_bpm(ppg_final, fs,window)
    
    print(list_bpm_final)
    print("Rata-Rata: "+ str(np.mean(list_bpm_final)))
    print("ref: "+ str(ref))
    print("durasi: "+str(duration))
    selisih = (np.mean(list_bpm_final))-ref
